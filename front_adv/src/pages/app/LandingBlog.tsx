  ...
  return (
    <div className="page-container space-y-6 animate-fade-in">
          ...
          <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
            <div className="space-y-3">
              <div className="inline-flex rounded-full border bg-background px-3 py-1 text-xs font-medium text-muted-foreground">
                Marketing juridico e gestao editorial
              </div>
              <div className="space-y-2">
                <h1 className="page-title">Blog Institucional</h1>
                <p className="max-w-3xl text-sm text-muted-foreground">
                  Planeje, publique e acompanhe os artigos do escritorio com foco em autoridade, SEO e relacionamento com novos clientes.
                </p>
              </div>
            </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
        ...
        <Card className="border-border/70 shadow-sm"><CardContent className="p-5"><p className="text-sm text-muted-foreground">Publicados</p><p className="mt-1 text-2xl font-semibold">{metrics.published}</p><p className="mt-1 text-xs text-muted-foreground">Conteudos visiveis no site publico.</p></CardContent></Card>
      ...
      {loading ? (
        ...
        <Card className="border-border/70 shadow-sm"><CardContent className="space-y-4 p-6"><div className="space-y-1"><h2 className="text-lg font-semibold">Nao foi possivel carregar os artigos</h2><p className="text-sm text-muted-foreground">Verifique a conexao com a API e tente novamente.</p></div><Button variant="outline" onClick={() => postsQuery.refetch()}>Tentar novamente</Button></CardContent></Card>
      ) : (
        <Card className="border-border/70 shadow-sm">
          <CardHeader className="border-b bg-muted/20">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><div><CardTitle>Artigos do blog</CardTitle><CardDescription>{filteredPosts.length} artigo(s) encontrado(s) com os filtros atuais.</CardDescription></div><div className="rounded-full border bg-background px-3 py-1 text-xs font-medium text-muted-foreground">Preview, duplicacao e publicacao rapida</div></div>
          </CardHeader>
          <CardContent className="p-0">
            {filteredPosts.length === 0 ? <div className="p-6"><EmptyState icon={BookOpenText} title="Crie artigos para atrair clientes" description="Use o blog para responder duvidas frequentes, fortalecer a autoridade do escritorio e ampliar sua presenca digital." action={canEdit ? { label: "Novo artigo", onClick: () => openPostDialog() } : undefined} /></div> : (
              <Table>
                <TableHeader>
                  <TableRow>
        ...
        <DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-6xl">
          <DialogHeader>
            <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
              <div><DialogTitle>{editingPostId ? "Editar artigo" : "Novo artigo"}</DialogTitle><DialogDescription>Monte artigos juridicos com foco em publicacao institucional, SEO e aproveitamento comercial.</DialogDescription></div>
          ...
          <Tabs defaultValue="basicos" className="space-y-6">
            <TabsList className="w-full justify-start overflow-auto"><TabsTrigger value="basicos">Dados basicos</TabsTrigger><TabsTrigger value="conteudo">Conteudo</TabsTrigger><TabsTrigger value="organizacao">Organizacao</TabsTrigger><TabsTrigger value="seo">SEO</TabsTrigger><TabsTrigger value="publicacao">Publicacao</TabsTrigger></TabsList>

            <TabsContent value="basicos" className="space-y-4">
              <div className="grid gap-4 md:grid-cols-2">
                <div className="space-y-2 md:col-span-2"><Label>Titulo do artigo</Label><Input value={postForm.title} onChange={(event) => setPostForm((previous) => ({ ...previous, title: event.target.value, slug: previous.slug ? previous.slug : buildSlug(event.target.value) }))} placeholder="Ex.: Como revisar contratos com seguranca juridica" /></div>
                ...
                <div className="space-y-2"><Label>Autor</Label><Input value={postForm.author_name} onChange={(event) => setPostForm((previous) => ({ ...previous, author_name: event.target.value }))} placeholder="Nome do responsavel pelo artigo" /></div>
            ...
            <TabsContent value="organizacao" className="space-y-4"><div className="grid gap-4 md:grid-cols-2"><div className="space-y-2"><Label>Categoria</Label><Input value={postForm.category} onChange={(event) => setPostForm((previous) => ({ ...previous, category: event.target.value }))} placeholder="Ex.: Direito Trabalhista" />{categories.length ? <div className="flex flex-wrap gap-2">{categories.map((category) => <Button key={category.slug} type="button" variant="outline" size="sm" onClick={() => setPostForm((previous) => ({ ...previous, category: category.label }))}>{category.label}</Button>)}</div> : null}</div><div className="space-y-2"><Label>Ordem de exibicao</Label><Input type="number" value={postForm.sort_order} onChange={(event) => setPostForm((previous) => ({ ...previous, sort_order: Number(event.target.value) || 0 }))} placeholder="0" /></div><div className="space-y-2"><Label>Tags</Label><Textarea value={postForm.tags_text} onChange={(event) => setPostForm((previous) => ({ ...previous, tags_text: event.target.value }))} rows={3} placeholder="Separe por virgula: bancos, revisao contratual, consumidor" /></div><div className="space-y-3"><div className="space-y-2"><Label>Galeria complementar</Label><Textarea value={postForm.gallery_text} onChange={(event) => setPostForm((previous) => ({ ...previous, gallery_text: event.target.value }))} rows={3} placeholder="Informe uma URL por linha para exibir imagens adicionais no artigo." /></div><div className="flex items-center justify-between rounded-2xl border px-4 py-3"><div className="space-y-1"><p className="font-medium">Artigo em destaque</p><p className="text-sm text-muted-foreground">Use para priorizar conteudos importantes na estrategia institucional.</p></div><Switch checked={postForm.is_featured} onCheckedChange={(checked) => setPostForm((previous) => ({ ...previous, is_featured: checked }))} /></div></div></div></TabsContent>

            <TabsContent value="seo" className="space-y-4"><div className="grid gap-4 md:grid-cols-2"><div className="space-y-2 md:col-span-2"><Label>Titulo para o Google</Label><Input value={postForm.seo_title} onChange={(event) => setPostForm((previous) => ({ ...previous, seo_title: event.target.value }))} placeholder="Titulo otimizado para busca" /></div><div className="space-y-2 md:col-span-2"><Label>Descricao para o Google</Label><Textarea value={postForm.seo_description} onChange={(event) => setPostForm((previous) => ({ ...previous, seo_description: event.target.value }))} rows={4} placeholder="Resumo com foco em clique e relevancia para o mecanismo de busca." /></div><div className="space-y-2 md:col-span-2"><Label>Palavras-chave</Label><Textarea value={postForm.seo_keywords_text} onChange={(event) => setPostForm((previous) => ({ ...previous, seo_keywords_text: event.target.value }))} rows={3} placeholder="Separe por virgula: juros abusivos, defesa do consumidor, contratos bancarios" /></div></div></TabsContent>

            <TabsContent value="publicacao" className="space-y-4"><div className="grid gap-4 md:grid-cols-2"><div className="space-y-2"><Label>Status do artigo</Label><Select value={postForm.publication_status} onValueChange={(value) => setPostForm((previous) => ({ ...previous, publication_status: value as BlogPublicationStatus }))}><SelectTrigger><SelectValue placeholder="Selecione o status" /></SelectTrigger><SelectContent><SelectItem value="RASCUNHO">Rascunho</SelectItem><SelectItem value="PUBLICADO">Publicado</SelectItem><SelectItem value="AGENDADO">Agendado</SelectItem></SelectContent></Select></div><div className="space-y-2"><Label>Data de publicacao</Label><Input type="datetime-local" value={postForm.published_at} onChange={(event) => setPostForm((previous) => ({ ...previous, published_at: event.target.value }))} /><p className="text-xs text-muted-foreground">O status agendado organiza a pauta editorial; a publicacao automatica depende da automacao do backend.</p></div></div></TabsContent>
          </Tabs>
